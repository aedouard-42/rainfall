...
This time we just a (huge) main function.
...
0x8049ab0 and 0x8049aac are two global variables
...
the address of the buffer starts at ESP+20

-> fgets(buffer, 128, stdin);


**Fgets return value check**
   0x080485b3 <+79>:	test   eax,eax
   0x080485b5 <+81>:	je     0x804872c <main+456>

If EAX == 0 goto <main+456>

--> if (fgets(...) == NULL)
       return 0;


**Compare the buffer with "auth "**
...
The cmps instruction compares esi to edi by substracting them.
The zero flag and carry flag are set according to the result.
The repz prefix means :
- increment esi and edi then repeat cmps as long as ECX != 0 and ZF = 0

-> strncmp(buffer, "auth ", 5)

**conditional jump to <main+222> if CF or ZF is not equal to zero**

  0x080485d1 <+109>:	seta   dl
   0x080485d4 <+112>:	setb   al

dl or al will be set to 1 if "strncmp" didn't return 0
...
copies the content of al(8 bit register) into EAX (32 bit register) and performs a sign extension

   0x080485e0 <+124>:	test   eax,eax
   0x080485e2 <+126>:	jne    0x8048642 <main+222>

if eax is not equal to zero , jump to <main+222>
...
**Set the position of the buffer to EAX+5 (after "auth ")**
   0x08048600 <+156>:	lea    eax,[esp+0x20]
   0x08048604 <+160>:	add    eax,0x5
...
For each iteration, scas compares al with the byte at [EDI], increments EDI and decrements ECX
repnz reiterates this operation as long as ECX != 0 and ZF = 0
...
If eax > 30 (0x1e) jump to <main+222>
...
Here another strncmp , this time  strncmp(buffer, "reset", 5);
...
if (strncmp(buffer, "reset", 5) != 0) jump to <main+276>
...
Another strncmp for a conditional jump.
This time:
...
strdup into g_ptr2 at position buffer + 7:
...
**Condition to system("/bin/sh")**

   0x080486e2 <+382>:	mov    eax,ds:0x8049aac              -> g_ptr1
   0x080486e7 <+387>:	mov    eax,DWORD PTR [eax+0x20]
   0x080486ea <+390>:	test   eax,eax
   0x080486ec <+392>:	je     0x80486ff <main+411>

If EAX (here gptr1[32]) == 0  -> jump to <main+411>
...
This makes the program run in a while (true) loop

Now from all these we can build the following C source code :
